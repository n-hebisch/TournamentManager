function createTournament(name) {
    if (name.search(',') === -1 && name !== '' && name !== undefined) {
        ...
        xhttp.open('POST', '/tournament', true);
...
function deleteTournamentById(ID) {
    ...
    xhttp.open('DELETE', '/tournament', true);
    xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhttp.send('id=' + ID)
}

//ändert den Namen des Spielers
function editTournamentname(name, ID) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            document.getElementsByTagName('html')[0].innerHTML = this.responseText;
    ...
    xhttp.open('PUT', '/tournament', true);
    xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhttp.send('name=' + name + '&id=' + ID)
}


function deleteTeamFromTournament(teamid,tournamentid){
    ...
    xhttp.open('DELETE', '/tournament/deleteTeamFromTournament', true);
    xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhttp.send('teamid=' + teamid+'&tournamentid='+tournamentid)
}
